...
import { getDurationInDays, formatDate, getCleanText, getLocationFromText, statusLog, getHostname, autoScroll, Location } from './utils';
...
export class LinkedInProfileScraper {
  ...
  public setup = async () => {
    ...
    try {
      ...
      // Skip login check for now - we'll check when actually scraping
  ...
  private createPage = async (): Promise<Page> => {
    ...
    try {
      ...
      page.on('request', (req) => {
        ...
        if (blockedResourcesByHost.includes(req.resourceType()) && hostname && (blockedHosts as any)[hostname] === true) {
      ...
      statusLog(logSection, `Setting session cookie using cookie: ${this.options.sessionCookieValue.substring(0, 10)}...`)
  ...
  private getBlockedHosts = (): object => {
    ...
    let blockedHostsObject = blockedHostsArray.reduce((prev: any, curr) => {
  ...
  public checkIfLoggedIn = async () => {
    ...
    try {
      ...
      statusLog(logSection, `Error during login check: ${err.message}`)
      throw new Error(`Failed to check login status: ${err.message}`);
    }
  };
   ...
   */
  public run = async (profileUrl: string): Promise<LinkedInProfile> => {
